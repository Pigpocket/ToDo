...
//  ToDoTests
//
//  Created by Tomas Sidenfaden on 8/2/18.
...
class DetailViewControllerTests: XCTestCase {
    ...
    func test_hasItemDescriptionLabel() {
        let itemDescriptionLabel = sut.itemDescriptionLabel?.isDescendant(of: sut.view) ?? false
        XCTAssertTrue(itemDescriptionLabel)
    }
    
    func test_HasMapView() {
        let mapViewIsSubView =
            sut.mapView?.isDescendant(of: sut.view) ?? false
